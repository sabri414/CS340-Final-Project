...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($Song_id_err)  && empty($Song_title_err) && empty($Artist_err) && empty($Record_err) && empty($Category_err) && empty($Language_err) && empty($Duration_err) && empty($Artist_id_err)){
        ...
        $sql = "INSERT INTO Song (Song_id, Song_Title, Record_Label, Artist, Language, Category, Duration, Artist_id) 
		        VALUES (?,?,?,?,?,?,?,?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "isssssii", $param_sid, $param_sname, $param_rl, $param_a, $param_l, $param_cat, $param_dur, $param_aid);
